...
- **Surfaces:** less is more. Radii are `--r-sm` / `--r-md` / `--r-lg` (6/8/10px); shadows are near zero. The targets stage has **no cards**: the anatomy sits in `.anatomy-stage` (most of the viewport), and the side column and estimate are sections divided by hairlines. `.card` (1px `--line-soft`, no shadow) remains on the Nutrition and Week plan stages until they are redesigned.
